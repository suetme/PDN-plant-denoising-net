...
class AddUniformBallNoise(object):  # Uniform Noise
...
def laplace_train_transforms(noise_std_min, noise_std_max, scale_d=0.2, rotate=True):
    transforms = [
        NormalizeUnitSphere(),
        AddLaplacianNoise(noise_std_min=noise_std_min, noise_std_max=noise_std_max),
        RandomScale([1.0 - scale_d, 1.0 + scale_d]),
    ]
    if rotate:
        transforms += [
...
def uniball_train_transforms(scale=0, scale_d=0.2, rotate=True):
    transforms = [
        NormalizeUnitSphere(),
        AddUniformBallNoise(scale=scale),
        RandomScale([1.0 - scale_d, 1.0 + scale_d]),
    ]
    if rotate:
        transforms += [
...
def discrete_train_transforms(scale=0, scale_d=0.2, rotate=True):
    transforms = [
        NormalizeUnitSphere(),
        AddDiscreteNoise(scale=scale),
        RandomScale([1.0 - scale_d, 1.0 + scale_d]),
    ]
    if rotate:
        transforms += [
